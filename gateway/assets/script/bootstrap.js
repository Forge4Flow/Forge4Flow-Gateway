...
var app = angular.module("faasGateway", [
  "ngMaterial",
  "ngMessages",
  "faasGateway.funcStore",
]);
...
app.controller("home", [
  ...
  function (
    ...
    $http.get("../system/namespaces").then(function (response) {
    ...
    var refreshFunction = function (functionInstance) {
      ...
      $http
        .get(buildNamespaceAwareURL(url, $scope.selectedNamespace))
        .then(function (response) {
          $scope.selectedFunction.ready =
            response.data &&
            response.data.availableReplicas &&
            response.data.availableReplicas > 0;
        })
        .catch(function (err) {
          console.error(err);
        });
    ...
    $scope.fireRequest = function () {
      var requestContentType =
        $scope.invocation.contentType == "json"
          ? "application/json"
          : "text/plain";
      ...
      var fnNamespace =
        $scope.selectedNamespace && $scope.selectedNamespace.length > 0
          ? "." + $scope.selectedNamespace
          : "";
      var options = {
    ...
    var refreshData = function (selectedNamespace) {
      $http
        .get(buildNamespaceAwareURL("/system/functions", selectedNamespace))
        .then(function (response) {
          const curNamespace =
            $scope.functions.length > 0 &&
            $scope.functions[0].namespace &&
            $scope.functions[0].namespace
              ? $scope.functions[0].namespace
              : "";
          const newNamespace =
            response.data && response.data[0] && response.data[0].namespace
              ? response.data[0].namespace
              : "";
          ...
          if (
            ...
            (curNamespace !== newNamespace ||
              $scope.functions.length != response.data.length)
          ) {
      ...
      $scope.annotationInputsToAnnotations = function () {
        ...
        for (var i = 0; i < self.annotationInputs.length; i++) {
          if (self.annotationInputs[i].key && self.annotationInputs[i].value) {
            result[self.annotationInputs[i].key] =
              self.annotationInputs[i].value;
          }
        }
